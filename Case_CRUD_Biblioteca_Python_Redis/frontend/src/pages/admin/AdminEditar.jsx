...
import { getLivro, atualizarLivro, uploadCover } from '../../services/api'
    ...
    try {
      ...
      if (coverFile) {
        try {
          ...
          setToast({ message: 'Livro atualizado, mas falha ao enviar a capa.', type: 'error' })
      ...
      const msg = err.response?.status === 401
        ...
        : (Array.isArray(detail) ? detail.map(d => d.msg).join(' | ') : detail || 'Erro ao atualizar.')
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate('/admin')} className="btn-secondary text-sm py-1.5">← Dashboard</button>
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Editar Livro</h1>
          <p className="text-sm text-slate-500 truncate max-w-xs">{livro ? livro.titulo : 'Carregando...'}</p>
        </div>
      </div>
      <div className="card">
        {fetching ? (
