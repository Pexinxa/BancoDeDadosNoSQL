...
JWT_SECRET=minha_chave_super_secreta_biblioteca_2024
JWT_EXPIRE_MINUTES=120
COVERS_DIR=/covers
LOAN_LIMIT=3
```

> **Atenção:** em produção, troque `JWT_SECRET` e `ADMIN_PASSWORD` por valores seguros e nunca os suba para o repositório.
...
│   ├── database.py       ← connection pool Redis
│   ├── models.py         ← schemas Pydantic
│   ├── repository.py     ← acesso a dados + lógica de empréstimos
│   ├── auth.py           ← JWT, bcrypt, dependências de role
│   └── main.py           ← rotas, middlewares, upload de capas
└── frontend/
    ├── Dockerfile
    ├── public/
    ...
    └── src/
        ...
        ├── layouts/
        │   ├── PublicLayout.jsx  ← header dinâmico (guest/user/admin)
        │   └── AdminLayout.jsx
        ├── pages/
        │   ├── auth/
        │   │   └── AuthPage.jsx  ← Login + Cadastro (abas)
        │   ├── public/
        │   │   ├── Catalogo.jsx  ← catálogo com hero, filtros, cards
        │   │   └── Cadastrar.jsx ← formulário de cadastro de livro
        │   └── admin/
        │       ├── AdminDashboard.jsx ← estatísticas + tabela
        │       └── AdminEditar.jsx    ← edição + upload de capa
        └── components/
            ├── BookForm.jsx      ← formulário reutilizável + upload de capa
            ├── ConfirmModal.jsx
            └── Toast.jsx
```
